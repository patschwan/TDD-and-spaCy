...
class TestNerClient(unittest.TestCase):
    ...
    # Client Behavior - spaCy gives back
    #   { ents: [{...}],
    #      html: "<span>..."}
    def test_get_ents_returns_dictionary_given_empty_string_causes_empty_spacy_doc_ents(self):
        model = NerModelTestDouble('eng')  # lightweight test double of spaCy
        model.returns_doc_ents([])  # returns an empty list
    ...
    # we're not testing spaCy - we're testing the ner-client
    # Think of: A) spAcy gives us A then B) we expect a condition to be meet
    ...
    def test_get_ents_given_spacy_LOC_is_returned_serializes_to_Group(self):
        model = NerModelTestDouble('eng')
    ...
    def test_get_ents_given_spacy_LANGUAGE_is_returned_serializes_to_Group(self):
        model = NerModelTestDouble('eng')
    ...
    def test_get_ents_given_spacy_GPE_is_returned_serializes_to_Group(self):
        model = NerModelTestDouble('eng')
    ...
    def test_get_ents_given_multiple_ents_serializes_all(self):
        model = NerModelTestDouble('eng')
        doc_ents = [{'text': 'Australia', 'label_':'GPE'}, {'text': 'Judith Polger', 'label_':'PERSON'}]
        model.returns_doc_ents(doc_ents)
        ner = NamedEntityClient(model)
        result = ner.get_ents('...')
        expected_result = {'ents': [{'ent': 'Australia', 'label': 'Location'}, 
        {'ent': 'Judith Polger', 'label': 'Person'}], 'html': ""}
        self.assertListEqual(result['ents'], expected_result['ents'])